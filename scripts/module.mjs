import {ID, catalog, saveCustom, state, purchase, purchaseError, lose, benefits, limit, CATALOG, claim, SITUATIONAL, practiceLimit, STARTER_IP, vote, votersFor, TIPS} from "./rules.mjs";
...
/* ---------- Tooltip: concept C. The bonus once, skills as a list, the condition as one line. ---------- */
const hiLite = (t) => escapeHTML(t).replace(/(\+\d+|−\d+eb|BODY \+\d+|BODY \d+|\d+,\d+eb|1d6\/2|1d6|2d6|\b\d+ LUCK)/g, "<b>$1</b>");
function tier(t, upgraded = false) {
  if (!t) return "";
  let h = `<div class="head">${upgraded ? `<span class="tag">Upgraded</span>` : ""}${hiLite(t.head)}</div>`;
  if (t.list) h += `<ul>${t.list.map((x) => `<li><b>${escapeHTML(t.plus ?? "+2")}</b> ${escapeHTML(x)}</li>`).join("")}</ul>`;
  if (t.cond) h += `<div class="cond">${hiLite(t.cond)}</div>`;
  return h;
}
function tooltipHtml(c, rank) {
  const t = TIPS[c.id];
  if (!t) return `<div class="nplh-tip"><div class="head">${escapeHTML(c.base)}</div>${c.custom ? `<ol>${(c.upgrades ?? []).map((u) => `<li>${escapeHTML(u)}</li>`).join("") || "<li>No upgrades defined.</li>"}</ol>` : `<div class="up"><div class="head"><span class="tag">Upgraded</span>${escapeHTML(c.upgrade)}</div></div>`}</div>`;
  let html = `<div class="nplh-tip">${tier(t.basic)}`;
  if (t.ladder) {
    const step = Math.max(0, rank - 1);
    html += `<div class="up"><div class="head"><span class="tag">Upgrades</span>In order${step ? `, the crew is on step ${step}` : ""}</div><ol class="ladder">${t.ladder.map((x, i) => `<li class="${i + 1 === step ? "now" : ""}${i + 1 < step ? " done" : ""}">${hiLite(x)}</li>`).join("")}</ol></div>`;
  } else if (t.up) html += `<div class="up">${tier(t.up, true)}</div>`;
  return html + "</div>";
}
